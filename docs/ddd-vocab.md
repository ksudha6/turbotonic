...
| Manufacturer Block | The vendor's name, address, and country rendered on the Packing List as "Shipper / Manufacturer" and on the Commercial Invoice as "Seller". The vendor aggregate is the manufacturer for PROCUREMENT POs. A distinct Manufacturer entity with its own manufacturer_id column on products is deferred to iter 106. | Shipment |
| Shipper's Declaration | Fixed text appended to the Commercial Invoice: "I declare that the information on this invoice is true and correct." Standard customs declaration. Signatory name and declaration date require schema additions deferred to iter 106. | Shipment |
