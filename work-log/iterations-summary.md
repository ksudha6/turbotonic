...
> Last updated: iter 064 closed on 2026-04-24 (Phase 4.0 UI revamp foundation in progress).
...
- Phase 4.0 primitive library (iters 062-064): design tokens in `global.css`, empty `(nexus)` SvelteKit layout group (AppShell deferred to iter 068), `/ui-demo` gallery route (not linked from nav). Twelve Svelte 5 primitives under `frontend/src/lib/ui/`: seven leaves (Button, StatusPill, ProgressBar, Input, Select, DateInput, Toggle) and five composites (FormField, PanelCard, AttributeList, FormCard, KpiCard). All use `ui-<name>` scoped outermost class. `frontend/tests/primitives.spec.ts` runs 14 regression tests. Input primitive surfaces `aria-invalid` via omit-when-false per W3C. No user-facing aggregate page consumes these yet.
