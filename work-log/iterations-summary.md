...
> Last updated: iter 083 closed on 2026-04-29. Tier 5 finishing pass on PO detail (metadata + invoices + rejection + activity).
...
- Phase 4.2 G-22 PO document attachment (queued as iter 084): SM/ADMIN/VENDOR upload signed countersigned PO into `entity_type='PO'` via the iter 035 file storage. New `canManagePOAttachments` helper, file-picker `Button`, `Documents` `PanelCard` between Activity and Rejection History, `DataTable` of file metadata. Brainstorm permission matrix + which file_type strings the first cut accepts before building.
- Legacy `frontend/src/lib/components/ActivityTimeline.svelte` and `frontend/src/lib/components/StatusPill.svelte` retire when `invoice/[id]` revamps. PO detail no longer consumes either after iter 083; both components are last-consumer-on-`/invoice/[id]`.
