...
> Last updated: iter 066 closed on 2026-04-24 (Phase 4.0 UI revamp foundation in progress).
...
- Phase 4.0 primitive library (iters 062-066): design tokens in `global.css`, empty `(nexus)` SvelteKit layout group (AppShell deferred to iter 068), `/ui-demo` gallery route (not linked from nav). Twenty-one Svelte 5 primitives under `frontend/src/lib/ui/`: 7 leaves (Button, StatusPill, ProgressBar, Input, Select, DateInput, Toggle), 5 composites (FormField, PanelCard, AttributeList, FormCard, KpiCard), 6 display+state (Timeline, ActivityFeed, LoadingState, EmptyState, ErrorState, ErrorBoundary), 3 table+headers (DataTable, PageHeader, DetailHeader). All use `ui-<name>` scoped outermost class (ErrorBoundary is a wrapper with no class). `frontend/tests/primitives.spec.ts` runs 22 regression tests. Input surfaces `aria-invalid` via omit-when-false. LoadingState honors `prefers-reduced-motion`. DataTable generic over `T extends { id: string }` with server-driven pagination. ErrorBoundary installed but not wired into a layout yet. No user-facing aggregate page consumes these yet.
