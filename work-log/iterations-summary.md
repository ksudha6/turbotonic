...
> Last updated: iter 067 closed on 2026-04-24 (Phase 4.0 UI revamp foundation in progress).
...
- Phase 4.0 primitive library (iters 062-067): design tokens in `global.css`, empty `(nexus)` SvelteKit layout group (AppShell deferred to iter 068), `/ui-demo` gallery route (not linked from nav). Twenty-two Svelte 5 primitives under `frontend/src/lib/ui/`: 7 leaves, 5 composites, 6 display+state, 3 table+headers, 1 shell (Sidebar). Plus `sidebar-items.ts` with explicit `Record<UserRole, SidebarItem[]>` role map (decoupled from `permissions.ts`). All use `ui-<name>` scoped outermost class. 24 primitive tests + 6 sidebar-items tests (30 total). Input surfaces `aria-invalid`. LoadingState honors `prefers-reduced-motion`. DataTable generic + server-driven pagination. ErrorBoundary installed but not wired into a layout yet. No user-facing aggregate page consumes these yet.
