...
> Last updated: iter 065 closed on 2026-04-24 (Phase 4.0 UI revamp foundation in progress).
...
- Phase 4.0 primitive library (iters 062-065): design tokens in `global.css`, empty `(nexus)` SvelteKit layout group (AppShell deferred to iter 068), `/ui-demo` gallery route (not linked from nav). Eighteen Svelte 5 primitives under `frontend/src/lib/ui/`: 7 leaves (Button, StatusPill, ProgressBar, Input, Select, DateInput, Toggle), 5 composites (FormField, PanelCard, AttributeList, FormCard, KpiCard), 6 display+state (Timeline, ActivityFeed, LoadingState, EmptyState, ErrorState, ErrorBoundary). All use `ui-<name>` scoped outermost class (ErrorBoundary is a wrapper with no class). `frontend/tests/primitives.spec.ts` runs 19 regression tests. Input primitive surfaces `aria-invalid` via omit-when-false per W3C. LoadingState honors `prefers-reduced-motion`. ErrorBoundary installed but not yet wired into a layout. No user-facing aggregate page consumes these yet.
