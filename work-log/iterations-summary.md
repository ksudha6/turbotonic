...
> Last updated: iter 063 closed on 2026-04-24 (Phase 4.0 UI revamp foundation in progress).
...
- Phase 4.0 primitive library (iters 062-063): design tokens in `global.css`, empty `(nexus)` SvelteKit layout group (AppShell deferred to iter 068), `/ui-demo` gallery route (not linked from nav), and seven Svelte 5 primitives under `frontend/src/lib/ui/` — Button, StatusPill, ProgressBar, Input, Select, DateInput, Toggle. Primitives use `ui-<name>` scoped outermost class to avoid colliding with pre-revamp global rules. `frontend/tests/primitives.spec.ts` runs 9 regression tests. No user-facing aggregate page consumes these yet.
