...
> Last updated: iter 068 closed on 2026-04-24 (Phase 4.0 UI revamp foundation in progress).
...
- Phase 4.0 primitive library (iters 062-068): design tokens in `global.css`, empty `(nexus)` SvelteKit layout group, `/ui-demo` gallery route + `/ui-demo/shell` AppShell preview (neither linked from nav). Twenty-five Svelte 5 primitives under `frontend/src/lib/ui/`: 7 leaves, 5 composites (incl. KpiCard with icon slot), 6 display+state, 3 table+headers, 4 shell (Sidebar with sections + footer + roleLabel, TopBar, AppShell). Plus `sidebar-items.ts` returning `Array<SidebarSection>` with a single Workspace section containing per-role items. All use `ui-<name>` scoped outermost class. `primitives.spec.ts`: 31 tests; `sidebar-items.spec.ts`: 7 tests (38 primitive-layer tests, 140 Playwright total including pre-revamp flows). Input surfaces `aria-invalid`. LoadingState honors `prefers-reduced-motion`. DataTable generic + server-driven pagination. AppShell composes Sidebar + TopBar + ErrorBoundary; mobile off-canvas drawer works at 390px. No user-facing aggregate page consumes these yet.
