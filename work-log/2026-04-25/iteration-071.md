...
- Hide the TopBar hamburger toggle at `≥769px` viewports. At desktop the sidebar is permanently visible via the `240px 1fr` grid; clicking the hamburger toggles internal `sidebarOpen` state with no visible effect. Add a `@media (max-width: 768px)` wrapper or a class that hides `.toggle` at desktop. `frontend/src/lib/ui/TopBar.svelte`.
- Make the desktop sidebar collapsible. Today the sidebar is fixed at 240px on desktop with no collapse affordance; the hamburger only meaningfully drives the mobile drawer. Add a desktop collapse mode (rail/icon-only or fully hidden) wired to the existing toggle, with state persisted across sessions. Decide rail-vs-hidden during the iter that tackles this — Lovable mock didn't show a collapsed state.
