...
- [x] Add `vendor_id` filter parameter to `list_pos_paginated()` (already has vendor_id filter, but this is the auth-level filter, not the UI filter)
...
- [x] Add vendor_id check to `get()`: after loading PO, verify `po.vendor_id == current_user.vendor_id` for VENDOR users

### Repository-level filtering: InvoiceRepository
- [x] Add `vendor_id` filter to `list_all()` -- join invoices -> purchase_orders to filter by PO's vendor_id
- [x] Add vendor_id check to `get_by_id()`: load invoice, then load its PO, verify vendor_id match for VENDOR users
- [x] Add vendor_id filter to `invoiced_quantities()` and `list_by_po()` -- verify PO belongs to vendor

### Repository-level filtering: MilestoneRepository
- [x] Add vendor_id check to `list_by_po()` -- verify PO belongs to vendor before returning milestones

### Repository-level filtering: ActivityLogRepository
- [x] Filter `list_recent()` for VENDOR users: only return entries where entity_id refers to a PO or invoice belonging to the vendor
...
- [x] Filter `unread_count()` similarly for VENDOR users
- [x] Filter `list_for_entity()`: verify the entity belongs to the vendor

### Router changes: pass current_user to repositories
- [x] Update `backend/src/routers/purchase_order.py`: inject current_user into list/get/detail calls
...
- [x] Update `backend/src/routers/invoice.py`: inject current_user
...
### Tests (scratch)
- [x] Log in as two different VENDOR users, compare PO list responses to verify disjoint sets

## Acceptance criteria
- [x] Users table has vendor_id column (nullable FK to vendors)
- [x] VENDOR users must have a vendor_id; SM/QUALITY_LAB/FREIGHT_MANAGER must not
- [x] VENDOR user listing POs sees only POs where `purchase_orders.vendor_id = user.vendor_id`
- [x] VENDOR user accessing another vendor's PO by ID gets 404 (not 403)
- [x] VENDOR user listing invoices sees only invoices linked to their vendor's POs
- [x] VENDOR user accessing another vendor's invoice by ID gets 404
- [x] VENDOR user activity feed and unread count reflect only their vendor's entities
- [x] VENDOR user dashboard data is scoped to their vendor
- [x] SM user sees all data across all vendors (no filtering)
- [x] QUALITY_LAB and FREIGHT_MANAGER see all data within their role's allowed endpoints
- [x] Vendor ownership checks use 404 (not 403) to avoid leaking entity existence
- [x] `make test` passes with all new and existing tests

## Notes

Added vendor-scoped data access as a cross-cutting filter across all repositories and routers. VENDOR users see only data linked to their vendor_id; non-VENDOR roles pass through unfiltered. Ownership checks return 404 to avoid leaking entity existence. Bulk operations (bulk_transition, bulk_invoice_pdf) skip non-owned entities silently rather than failing the whole request. The `check_vendor_access` helper in dependencies.py centralizes the ownership check. 13 permanent tests cover isolation between two vendors and SM full-visibility. One pre-existing test failure in test_invoice.py (date range filter) is unrelated.
